  ...
  return (
    <button onClick={onClick} className="w-full text-left px-3 py-2.5 rounded-[5px] hover:bg-hover transition-colors cursor-pointer group" style={{ transitionDuration: '80ms' }}>
      <div className="flex items-start gap-2">
        ...
        <span className="text-[13px] font-serif text-text-secondary group-hover:text-text line-clamp-2 leading-tight transition-colors" style={{ transitionDuration: '80ms' }}>{displayName}</span>
      </div>
    </button>
  );
}
