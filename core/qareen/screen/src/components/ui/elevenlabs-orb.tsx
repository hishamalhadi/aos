...
function Scene({
  ...
  // Sync refs on EVERY render — useFrame reads these, not closure vars
  manualInRef.current = manualInput ?? 0
  manualOutRef.current = manualOutput ?? 0
  // Store callback refs so useFrame always reads the latest function
  const getInputVolRef = useRef(getInputVolume)
  const getOutputVolRef = useRef(getOutputVolume)
  getInputVolRef.current = getInputVolume
  getOutputVolRef.current = getOutputVolume
  ...
  useEffect(() => {
    // Qareen is always dark — set inverted to 1
  ...
  useFrame((_, delta: number) => {
    ...
    if (modeRef.current === "manual") {
      targetIn = clamp01(
        manualInRef.current || inputVolumeRef?.current || getInputVolRef.current?.() || 0
      )
      targetOut = clamp01(
        manualOutRef.current || outputVolumeRef?.current || getOutputVolRef.current?.() || 0
      )
    } else {
  ...
  const uniforms = useMemo(() => {
    ...
    return {
      ...
      uInverted: new THREE.Uniform(1), // always dark
