  ...
  return (
    <div className="flex items-center gap-2 mb-3">
      ...
      {count !== undefined && count > 0 && (
        <span className="text-[10px] font-[510] text-text-quaternary bg-bg-tertiary rounded-[3px] px-1.5 py-0.5 leading-none">{count}</span>
      )}
    </div>
  );
}
...
export default function Home() {
  ...
  const { topTasks, activeTasks, todoTasks, recentDone } = useMemo(() => {
    ...
    const todo = allTasks.filter(t => t.status === 'todo').sort((a, b) => a.priority - b.priority);
    const done = allTasks
  ...
  return (
    <div>
      ...
      <p className="text-[11px] text-text-quaternary mt-1 mb-10">{dateStr}</p>
      ...
      <div className="mb-8">
        ...
        {isLoading ? (
          ...
          topTasks.map((task) => (
            <div key={task.id} className="h-9 flex items-center gap-3 hover:bg-hover rounded-[4px] px-3 -mx-3 transition-colors"
              style={{ transitionDuration: 'var(--duration-instant)' }}>
              <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${
                task.status === 'active' ? 'bg-green' : task.priority <= 2 ? 'bg-text-tertiary' : 'bg-text-quaternary'
              }`} />
      ...
      <div className="mb-8">
        ...
        {isLoading ? (
          ...
          inbox.map((item) => (
            <div key={item.id} className="h-9 flex items-center justify-between hover:bg-hover rounded-[4px] px-3 -mx-3 transition-colors"
              style={{ transitionDuration: 'var(--duration-instant)' }} title={item.text}>
              <span className="text-[13px] text-text-secondary truncate flex-1">{truncate(item.text, 80)}</span>
      ...
      {recentDone.length > 0 && (
        <div className="mb-8">
          ...
          {recentDone.map((task) => (
            <div key={task.id} className="h-9 flex items-center gap-3 px-3 -mx-3">
              ...
              <span className="text-[10px] text-text-quaternary shrink-0">{task.completed ? timeAgo(task.completed) : ''}</span>
            </div>
          ))}
        </div>
      )}
          ...
          <div className="flex flex-wrap gap-x-5 gap-y-2 px-3 -mx-3">
            {services.map((svc) => (
              <div key={svc.name} className="flex items-center gap-1.5">
                <span className={`w-[6px] h-[6px] rounded-full ${svc.status === 'running' || svc.status === 'online' ? 'bg-green' : 'bg-red'}`} />
                <span className="text-[11px] text-text-quaternary">{svc.name}</span>
              </div>
            ))}
          </div>
        ) : null}
